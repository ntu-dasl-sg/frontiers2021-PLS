...
Hazard model source: Wei 2018, Combined deterministic simulation (low frequency) and stochastic simulations (high frequency) for a few stations located around and within Kathmandu. We use a simplified 3D velocity model for the Kathmandu basin
...
Data received from Wei Shengji and Mei in July 2020
Data is re-analyzed to be in the same resolution as USGS Shakemap
Resolution: 0.0167 deg
Units: (cm/s)/s
...
```{r}
# Read from folder
pga <- read.table( here( "data","shaking_model", "deterministic_shengji,meng_denser", "pga.data"), head=TRUE, sep=" ")
# Change column names
colnames(pga) <- c("lon", "lat", "pga")
# Convert (cm/s)/s to g-units
pga$pga <- pga$pga * 0.0010197162129779282 #1 (cm/s)/s = 0.0010197162129779282 g
summary(pga$pga)
```
...
write.csv(pga.at.points,
          file= here("data", "building_port_70.csv"),
          row.names = F)
...
ggplot(data=frag_all.melt)+
  ...
  xlim(c(0,1))+
    ...
    theme(panel.background = element_rect(fill = NA),
          ...
          axis.line = element_line(colour = "black"))
# Save file
ggsave(file= here("graphics", "fragility_allcurves_v2_closer.png"),
       width = 5, height = 3,
       scale =1.5,
       dpi = 300)
```
...
FRAGILITY CURVES CLOSER - LESS LEGEND

```{r}

#as.factor(all_unret$class)
#levels(frag_all.melt$Building_type) <- c("B-", "B", "K5", "C2", "C3")
...
ggplot(data=frag_all.melt)+
  ...
  xlim(c(0,1))+
    ...
    theme(panel.background = element_rect(fill = NA),
          ...
          axis.line = element_line(colour = "black"))
# Save file
ggsave(file= here("graphics", "data-fragility.png"),
       width = 5, height = 3,
       scale =1.5,
       dpi = 300)
```
...
ggplot(all_unret, aes(y=retrofit, x=occ_day)) +
...
ggsave(file= here("graphics", "case1_occupancy.png") ,
       width = 5.5, height = 2,
       scale =1,
       dpi = 300,
       bg = "transparent")
...
ggsave(file= here("graphics", "case1_occ_violin.png") ,
       width = 5.5, height = 2,
       scale =1,
       dpi = 300,
       bg = "transparent")
...
#as.factor(all_unret$class)
# levels(all_unret$class) <- c("Bm", "B", "K5", "C2")
...
#all_unret$class <- factor(c("Bm", "B", "K5", "C2"))
...
ggsave(file= here("graphics", "case1_bldgtype.png") ,
       #plot = last_plot(),
       bg = "white",
       width = 5, height = 3,
       scale =1.2,
...
write.csv(all_unret,
          file= here("data", "building_port_70_class.csv"),
          row.names = F)
```
...
```{r}
load(here("RData_case1","c1.collapse.RData"))
load(here("RData_case1","c2.collapse.RData"))
load(here("RData_case1","unret_fat.RData"))
load(here("RData_case1","ret_fat.RData"))
load(here("RData_case1","c1.occ.RData"))
bldg_count <- length(c1.occ)
fatality_rate = 0.2
n = 10000

library(formattable) # for function comma()
```
...
ggplot(fatality.melt, aes(x=Fatalities, y=Building_state, fill=Building_state, color=Building_state)) +
  ...
  geom_segment(aes(x = mean_unret_fat, xend = mean_unret_fat, y=1, yend=1.53),
               linetype="dotted", 
               size=0.4, color = "grey80", alpha=0.1)+
  geom_segment(aes(x = mean_ret_fat, xend = mean_unret_fat, y=1.53, yend=1.53),
               arrow = arrow(length = unit(0.5, "cm")), linetype=1, size=0.4, color = "black")+
  geom_segment(aes(x = mean_unret_fat, xend = mean_ret_fat, y=1.53, yend=1.53),
               arrow = arrow(length = unit(0.5, "cm")), linetype=1, size=0.4, color = "black")+
  annotate(geom="text", x=205, y=1.70, label=paste("Estimated lives saved =", LivesSaved), color="black", angle=0) +
  annotate(geom="text", x=550, y=2.8, label="Estimates based on 10,000 simulations" , color="grey20", angle=0) +
  annotate(geom="text", x=495, y=2.65, label=paste("Assumed fatality rate =", fatality_rate), color="grey20", angle=0) +
  annotate(geom="text", x=498, y=2.5, label=paste("Total occupancy =", comma(tot.occ)), color="grey20", angle=0) +
  annotate(geom="text", x=575, y=2.35, label=paste("Fatalities if all buildings collapsed =", comma(tot.fat)), color="grey20", angle=0) +
  annotate(geom="text", x=mean_unret_fat+23, y=1.1, label=paste(round(mean_unret_fat)), color="white", angle=0,  label.size = NA) +
  annotate(geom="text", x=mean_ret_fat+17, y=2.1, label=paste(round(mean_ret_fat)), color="white", angle=0, label.size = NA) +
  annotate(geom="text", x=mean_unret_fat+30, y=1.2, label="mean", color="white", angle=0,  label.size = NA) +
  annotate(geom="text", x=mean_ret_fat+30, y=2.2, label="mean", color="white", angle=0,  label.size = NA) +
  xlim(c(0,800))+
  labs(y = "Scenario",
       title = paste("Estimated fatalities for", bldg_count, "school buildings"))+ 
...
ggplot(collapse.melt, aes(x=Collapsed_Buildings, y=Building_state, fill=Building_state, color=Building_state)) +
  ...
  geom_segment(aes(x = mean_ret_coll, xend = mean_ret_coll, y=1, yend=2),
               linetype="dotted", 
               size=0.4, color = "grey80", alpha=0.1)+
    geom_segment(aes(x = mean_unret_coll, xend = mean_unret_coll, y=1, yend=1.65),
               linetype="dotted", 
               size=0.4, color = "grey80", alpha=0.1)+
  geom_segment(aes(x = mean_ret_coll, xend = mean_unret_coll, y=1.65, yend=1.65),
               arrow = arrow(length = unit(0.5, "cm")), linetype=1, size=0.4, color = "black")+
  geom_segment(aes(x = mean_unret_coll, xend = mean_ret_coll, y=1.65, yend=1.65),
               arrow = arrow(length = unit(0.5, "cm")), linetype=1, size=0.4, color = "black")+
  #geom_vline(xintercept = mean_ret_coll, alpha = 0.15)+
  #geom_vline(xintercept = mean_unret_coll, alpha = 0.15)+
  annotate(geom="text", x=7.5, y=1.75, label=paste("Avoided collapse =", CollapseSaved), color="black", angle=0) +
  annotate(geom="text", x=19, y=2.8, label="Estimates based on 10,000 simulations" , color="grey20", angle=0) +
  annotate(geom="text", x=mean_unret_coll+0.5, y=1.1, label=paste(round(mean_unret_coll,0)), color="white", angle=0) +
  annotate(geom="text", x=mean_ret_coll+0.4, y=2.1, label=paste(round(mean_ret_coll,0)), color="white", angle=0) +
  #annotate(geom="text", x=mode_unret_coll+1, y=1.2, label="mode", color="white", angle=0) +
  #annotate(geom="text", x=mode_ret_coll-.5, y=2.2, label="mode", color="white", angle=0) +
  ...
  labs(y = "Scenario",
       ...
       title = paste("Estimated building collapse out of", bldg_count, "school buildings"))+ 
  theme_ridges(center = TRUE, grid = FALSE)

ggsave(file= here("graphics", "s1_collapsedbldgs_case1.png") ,
       #plot = last_plot(),
       bg = "white",
       width = 7, height = 3,
       scale =1.2,
...
save(c1.collapse, file = here("RData_case1","c1.collapse.RData"))
save(c2.collapse, file = here("RData_case1","c2.collapse.RData"))
save(unret_fat, file = here("RData_case1","unret_fat.RData"))
save(ret_fat, file = here("RData_case1","ret_fat.RData"))
save(c1.occ, file = here("RData_case1","c1.occ.RData"))
