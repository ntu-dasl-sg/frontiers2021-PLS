# CASE 1 - using Shakemap as hazard
...
r.pga <- raster(here( "data","shaking_model","shakemap", "pga_mean.flt"), sep=",")
crs(r.pga) <- crs(map_proj)
r.pga <- 10^r.pga # File is in natural log units 
# Convert raster to dataframe for plotting with ggplot
pts.raster <- rasterToPoints(r.pga)  # change to rasterToPoints(r.pga) for a full map
...
########
# FATALITY CALCULATION - COUNTERFACTUAL
########
...
ggplot(fatality.melt, aes(x=Fatalities, y=Building_state, fill=Building_state)) +
        ...
        ggsave(file= here("graphics", "4b_case1-ShakeMap.png") ,
               #plot = last_plot(),
               bg = "white",
               width = 7, height = 3,
               scale =1.2,
